...
const tealColor = computed(() =>
  mode.value === "dark" ? JASTEL_BRAND_COLORS.WORDMARK_TEL_DARK : JASTEL_BRAND_COLORS.WORDMARK_TEL_LIGHT,
);
...
<template>
  <div class="flex items-center gap-3" :style="{ height: `${height}px` }">
    <svg
      :style="{ height: `${height}px`, width: 'auto', overflow: clipOrbit ? 'hidden' : 'visible' }"
      :viewBox="viewBox"
      fill="none"
...
@keyframes jtShipOrbit {
  0% {
    transform: rotate(0deg) translate(42px) rotate(0deg);
    opacity: 1;
  }
  50% {
    transform: rotate(180deg) translate(42px) rotate(-180deg);
    opacity: 0.4;
  }
  100% {
